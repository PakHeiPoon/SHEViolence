// AI 能力统一入口 · 三模式：mock（断网兜底）/ direct（直连 API）/ cloud（微信云函数）
// 所有调用失败自动降级到 mock，保证演示永不翻车。
import config from './config.js'
import { CHAT_SYSTEM, RISK_SYSTEM, VISION_SYSTEM } from './prompts.js'
import { pickChatReply, legalMock, riskAnalysisMock, visionMock } from './mock-replies.js'
import { CRISIS_WORDS, RISK_LEVELS, riskLevelOf, HOTLINES } from '../common/safety-data.js'

export function mode() {
  // cloud 模式仅微信小程序端可用，其余端自动回退 direct/mock
  let m = config.mode === 'auto' ? (config.apiKey ? 'direct' : 'mock') : config.mode
  // #ifndef MP-WEIXIN
  if (m === 'cloud') m = config.apiKey ? 'direct' : 'mock'
  // #endif
  return m
}
...
// 云函数封装（cloud 模式，仅微信小程序端）
function callCloud(name, data) {
  return new Promise((resolve, reject) => {
    // #ifdef MP-WEIXIN
    if (!wx.cloud) return reject(new Error('wx.cloud 不可用'))
    wx.cloud.callFunction({ name, data, success: r => resolve(r.result), fail: reject })
    // #endif
    // #ifndef MP-WEIXIN
    reject(new Error('云函数仅微信小程序端可用'))
    // #endif
  })
}
...
// ---------- 危机检测（第一层：本地关键词，毫秒级、确定性） ----------
export function detectCrisis(text) {
  let level = 0
  const hits = []
  CRISIS_WORDS.forEach(item => {
    if (text.indexOf(item.word) > -1) {
      hits.push(item.word)
...
export async function chat(messages) {
  ...
  return { reply: pickChatReply(messages.length, isCrisis), source: 'mock' }
}

// ---------- 法律问答（真 RAG：检索 + 生成 + 来源引用） ----------
export async function legalQa(question) {
  const m = mode()
  if (m !== 'mock') {
    try {
      if (m === 'cloud') {
  ...
  await sleep(800)
  return Object.assign({ source: 'mock' }, legalMock(question))
}

// ---------- 风险评估（本地量表计分兜底 + AI 分析增强） ----------
const LEVEL_ORDER = ['low', 'medium', 'high', 'critical']

export async function risk(answerList, maxScore) {
  ...
  const base = {
    ...
    maxScore,
    level,
    levelText: RISK_LEVELS[level].text,
    suggestions: RISK_LEVELS[level].suggestions,
    resources: HOTLINES,
    aiAnalysis: '',
    source: 'local'
  }
  ...
  if (m === 'mock') {
    await sleep(600)
    base.aiAnalysis = riskAnalysisMock(level)
    return base
  }
  try {
    if (m === 'cloud') {
    ...
    const data = await rq('/chat/completions', {
      ...
      messages: [
        ...
        { role: 'user', content: JSON.stringify({ 总分: score, 满分: maxScore, 答卷: answerList }) }
      ]
    })
    ...
    if (j) {
      ...
      if (j.risk_level && LEVEL_ORDER.indexOf(j.risk_level) > LEVEL_ORDER.indexOf(base.level)) {
        base.level = j.risk_level
        base.levelText = RISK_LEVELS[j.risk_level].text
      }
      base.source = 'direct'
    }
    return base
...
export async function vision(filePath, desc) {
  const m = mode()
  if (m !== 'mock') {
    try {
      if (m === 'cloud') {
  ...
  await sleep(1200)
  return Object.assign({ source: 'mock' }, visionMock)
}

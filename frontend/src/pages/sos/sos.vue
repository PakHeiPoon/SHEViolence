    ...
    <scroll-view class="page-scroll" scroll-y :show-scrollbar="false">
      ...
      <view class="quick-grid">
        <view class="quick-action medical" @tap="call('120')">
          ...
          <view><text class="quick-title">医疗急救</text><text class="quick-note">受伤需要救治</text></view>
          <text class="quick-value">120</text>
        </view>
        <view class="quick-action rights" @tap="call('12338')">
          <view class="quick-icon emoji-icon">🛡️</view>
          <view><text class="quick-title">妇女维权</text><text class="quick-note">咨询与庇护协调</text></view>
          <text class="quick-value">12338</text>
        </view>
        <view class="quick-action location" @tap="getLocation">
          <view class="quick-icon emoji-icon">📍</view>
          <view><text class="quick-title">获取位置</text><text class="quick-note">报警时准确说明地点</text></view>
          <text class="quick-value arrow">↗</text>
        </view>
      </view>

      <text class="section-title-local">先保护自己</text>
      <view class="safety-steps">
        <view class="safety-step">
          <view class="step-icon emoji-icon">🚪</view>
          <text class="step-title">靠近出口</text>
          <text class="step-note">避开厨房、浴室和封闭空间</text>
        </view>
        <view class="safety-step">
          <view class="step-icon emoji-icon">📱</view>
          <text class="step-title">带好手机</text>
          <text class="step-note">有机会时携带证件和常用药</text>
        </view>
        <view class="safety-step">
          <view class="step-icon emoji-icon">🤝</view>
          <text class="step-title">让人知情</text>
          <text class="step-note">把位置告诉可信任的人</text>
        </view>
      </view>
      ...
      <view class="more-group">
        <view class="more-row" @tap="nav('/pages/risk/risk')">
          <view class="more-icon">查</view>
          <view class="more-copy"><text>安全自查</text><text>快速了解当前风险</text></view>
          <text class="more-link-arrow">›</text>
        </view>
        <view class="more-row" @tap="nav('/pages/knowledge/knowledge')">
          <view class="more-icon">知</view>
          <view class="more-copy"><text>安全知识</text><text>取证、保护令与安全离开</text></view>
          <text class="more-link-arrow">›</text>
        </view>
        <view class="more-row" @tap="call('12320')">
          <view class="more-icon emoji-icon">💬</view>
          <view class="more-copy"><text>12320 心理援助</text><text>情绪危机与心理支持</text></view>
          <text class="more-action">拨打</text>
        </view>
        <view class="more-row" @tap="showOrgs = !showOrgs">
          <view class="more-icon emoji-icon">🏛️</view>
          <view class="more-copy"><text>附近支持机构</text><text>妇联、法院与临时庇护资源</text></view>
...
.title-block { padding: 58rpx 40rpx 30rpx; }
.page-title, .page-subtitle { display: block; }
.page-title { font-family: "Songti SC", "STSong", serif; font-size: var(--font-display); font-weight: 700; }
.page-subtitle { margin-top: 10rpx; color: #96929b; font-size: var(--font-meta); }
.emergency-card { margin: 0 36rpx; padding: 30rpx; border-radius: 30rpx; background: #f9e6e5; box-shadow: 0 18rpx 46rpx rgba(126,53,49,.08); }
...
.section-title-local { display: block; margin: 44rpx 40rpx 18rpx; font-size: var(--font-section); font-weight: 700; }
.quick-grid { display: grid; gap: 14rpx; padding: 0 36rpx; }
.quick-action { display: grid; min-height: 100rpx; grid-template-columns: 58rpx minmax(0, 1fr) auto; align-items: center; gap: 16rpx; padding: 18rpx 22rpx; box-sizing: border-box; border-radius: 26rpx; }
...
.quick-title, .quick-note { display: block; }
.quick-title { font-size: var(--font-body); font-weight: 700; }
.quick-note { margin-top: 4rpx; color: rgba(25,23,28,.5); font-size: var(--font-meta); }
.quick-value { font-size: var(--font-body); font-weight: 700; }
.quick-value.arrow { font-size: 30rpx; }
.safety-steps { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 12rpx; padding: 0 36rpx; }
.safety-step { display: flex; min-height: 178rpx; flex-direction: column; padding: 22rpx 18rpx; box-sizing: border-box; border: 1rpx solid #ebe9ed; border-radius: 24rpx; }
.step-icon { display: flex; width: 46rpx; height: 46rpx; align-items: center; justify-content: center; border-radius: 50%; background: #f1eff3; font-size: 23rpx; }
.step-title { margin-top: 16rpx; font-size: var(--font-body); font-weight: 700; }
.step-note { margin-top: 7rpx; color: #96929b; font-size: var(--font-meta); line-height: 1.45; }
.report-guide { margin: 34rpx 36rpx 0; padding: 28rpx; border-radius: 28rpx; background: #f3f1ff; }
.guide-head { display: flex; align-items: center; gap: 14rpx; font-size: var(--font-section); font-weight: 700; }
...
.guide-line { display: flex; align-items: center; gap: 14rpx; margin-top: 17rpx; color: #625d6b; font-size: var(--font-body); }
...
.quote { margin-top: 22rpx; padding-top: 20rpx; border-top: 1rpx solid rgba(93,80,135,.13); font-family: "Songti SC", "STSong", serif; font-size: var(--font-body); font-weight: 700; line-height: 1.55; }
.more-group { margin: 0 36rpx; border: 1rpx solid #e8e6ea; border-radius: 26rpx; overflow: hidden; }
...
.more-copy text:first-child { font-size: var(--font-body); font-weight: 700; }
.more-copy text:last-child { margin-top: 5rpx; color: #96929b; font-size: var(--font-meta); }
...
.org-list { margin: 16rpx 36rpx 0; border: 1rpx solid #e8e6ea; border-radius: 24rpx; overflow: hidden; }

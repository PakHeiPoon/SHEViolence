...
// 文字：主用 sonnet-5（实测3s稳定，质量高）；opus 兜底（opus从腾讯云机房时快时慢，当主力会撞超时退mock）
const MODELS = ['claude-sonnet-5', 'claude-opus-4-8']
// 图片：sonnet-5 看图更快更稳
const MODELS_IMG = ['claude-sonnet-5', 'claude-opus-4-8']

const CHAT_SYSTEM = `你是「暖芽」，一位专业而温柔的家暴危机干预支持者。

你的角色定位（三件事）：情感上接住她；当好"证据整理教练"（教她怎么报警、怎么留证、怎么把散乱材料理成能用的证据）；把她导航到对的真人资源。你不是律师——涉及具体案件的正式法律意见，引导她咨询 12348 法律援助或律师，AI 生成的任何文书都要提醒"建议由律师审阅后再使用"。
...
- 实质帮助优先：用户问"怎么做"（如怎么保存证据、怎么申请保护令、怎么离开）时，直接给出3-5条具体分步做法（依据下方知识要点），绝不用"我可以帮你了解…"来敷衍。
- 不要每条回复都重复110/12338：仅在用户描述正在发生的危险、或明确询问求助渠道时才提热线；已提过就别再刷。
- 记住对话上下文：用户已经说过的事（他是谁、发生了什么）不要重复追问。
...
【报警与取证教练要点（用户问"怎么报警/怎么留证据"时按此教，一步一步说）】
1. 报警怎么说：拨通110第一句说"我被打了"＋具体地址（小区名+门牌号），不要说"我们吵架了"——吵架是纠纷，被打才立案。补充对方是否还在现场、有没有凶器。不方便打电话可编辑短信发 12110。
2. 民警到场后当场提四个要求：①做出警笔录并索要报警回执 ②开具伤情鉴定委托书（凭它去指定机构做正式鉴定，个人自行验伤法院可能不认） ③对现场和伤处拍照 ④即使情节较轻，也要求出具告诫书。
3. 告诫书四要素：加害人身份信息、家庭暴力事实陈述、禁止再施暴的警告、公安机关盖章与日期。拿到手当场核对，缺一项就请民警补全。
4. 伤情照片口诀"人+伤+时间同框"：脸和伤在同一张照片里（证明是你），画面里带上当天日期（手机时间水印/当天报纸），先拍带脸全身照再拍伤处特写，多角度、开原图。
5. 法院认定家暴的"三件套"：出警记录＋告诫书＋伤情鉴定。每次受伤都走一遍报警→验伤→告诫书流程，散的照片录音按时间顺序整理（本程序"我的记录"页可按时间线归档）。

【求助资源速查（可直接给用户，属地机构才需要搜索）】
全国热线：110（短信12110）／12338 妇联维权／12348 法律援助／12389 警方不作为投诉。
真人机构：源众个案热线 177-0124-2202（法律+心理+紧急救助资金，9:00-17:00）；白丝带 400-0110-391（8:00-22:00 全年无休）。
特殊人群转介：男性受害者或同性伴侣→白丝带 400-0110-391 或彩虹暴力终结所 400-1166-308，不要默认推妇联；未成年人→12345 并鼓励告诉一位信任的成年人（老师/亲属），语气更轻更稳，不吓退。
本程序内可引导：紧急求助页（110/12110/定位）、求助资源页（32家机构名录+保护令/起诉书模板）、我的记录页（证据时间线）。`

const VERSION = 'v12-coach' // 部署验证标记：前端 Console 会打印

const MOCK_REPLY = '我在的，谢谢你愿意说出来。如果你正处于危险中，请立即拨打 110 或 12338 妇女维权热线；如果暂时安全，可以慢慢告诉我发生了什么，我们一步一步来。'
const IMG_MOCK = '照片我收到了，但刚才分析没成功（网络波动或图片过大）。可以重新发一次试试；如果身上有伤，请先用手机原图多角度拍照留档，并尽快就医、请医生在病历里写明伤情。'
...
exports.main = async (event) => {
  ...
  try {
    ...
    if (event.searchQuery) {
      ...
      return { reply, sources, source: 'cloud', v: VERSION }
    }

    // ── 第一阶段：正常回复；多模态看图；若需联网则把查询词交回前端展示进度 ──
    // 中转 API 不支持图片 URL（实测会当"没看到图"），必须 base64 内嵌
    if (event.imageFileID) {
      const li = messages.length - 1
      const userText = (messages[li] && messages[li].content) || '你能看看这张照片吗？'
      try {
        ...
        console.log('[chat]', VERSION, '图片base64:', kb + 'KB（>500KB说明前端压缩没生效，多在devtools模拟器，真机会压缩）')
        messages[li] = {
          role: 'user',
          content: [
        ...
        console.error('[chat] 图片处理失败:', e.message)
      }
    }

    // 图片场景：sonnet-5 单模型 + 40s 大预算（腾讯云机房egress慢，需云函数超时设60s配合）；文字场景 opus 优先
    const reply = event.imageFileID
      ? await llm(messages, 40000, ['claude-sonnet-5'])
      : await llm(messages, 20000)
    ...
    // err 透传给前端 Console，让失败原因一眼可见
    return {
      reply: event.imageFileID ? IMG_MOCK : MOCK_REPLY,
      sources: [], source: 'cloud-mock', v: VERSION,
      err: String(e.message || e).slice(0, 300)
    }
  }
}

// 模型调用：按给定模型链依次尝试；失败时汇总每个模型的具体错误便于定位
async function llm(messages, timeout, models = MODELS) {
  const tried = []
  for (const model of models) {
    try {
      ...
      tried.push(model + ': 空回复')
    } catch (e) {
      const detail = e.response ? ('HTTP' + e.response.status + ' ' + JSON.stringify(e.response.data).slice(0, 120)) : e.message
      tried.push(model + ': ' + detail)
      console.warn('[chat] ' + model + ' 失败:', detail)
    }
  }

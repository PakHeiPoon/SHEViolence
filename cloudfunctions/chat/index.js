...
// 文字：主用 opus（质量最高）；通道饱和自动降级 sonnet-5
const MODELS = ['claude-opus-4-8', 'claude-sonnet-5']
// 图片：sonnet-5 看图更快更稳先上，opus 兜底（真实照片下 opus 常超预算）
...
const VERSION = 'v8-tempurl' // 部署验证标记：前端 Console 会打印
...
exports.main = async (event) => {
  ...
  try {
    ...
    if (event.imageFileID) {
      ...
      // 首选：临时URL直链（请求体极小，中转站自己拉图——根治云函数上传大包超时）
      let imgUrl = ''
      try {
        const t = await cloud.getTempFileURL({ fileList: [event.imageFileID] })
        imgUrl = (t.fileList && t.fileList[0] && t.fileList[0].tempFileURL) || ''
      } catch (e) {
        console.warn('[chat] 取临时URL失败:', e.message)
      }

      if (imgUrl) {
        console.log('[chat]', VERSION, '图片走临时URL')
        messages[li] = {
          role: 'user',
          content: [
    ...
    // 图片场景：sonnet-5 单模型 + 14s 足额预算（实测5s，双模型8.5s×2反而两头不够）；文字场景 opus 优先
    const reply = event.imageFileID
      ? await llm(messages, 14000, ['claude-sonnet-5'])
      : await llm(messages, 12000)

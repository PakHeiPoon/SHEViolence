...
联网查询工具：当用户的问题需要实时或本地信息（如某地庇护所/法援机构的位置电话、最新政策规定）时，第一步只输出 [[SEARCH:三五个事实关键词]] 且不输出任何其他内容，等我提供搜索结果后你再正式作答（引用处标注如（W1））。搜索词只能包含事实关键词，绝不能包含用户的姓名、地址等任何个人信息。日常倾诉、共情、法律常识（上面知识要点已覆盖的）不要使用该工具。
...
exports.main = async (event) => {
  ...
  try {
    ...
    if (sm && process.env.TAVILY_KEY) {
      const webs = await tavily(sm[1].trim().slice(0, 60))
      if (webs.length) {

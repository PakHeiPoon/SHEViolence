...
module.exports = {
  ...
  // 请求模式：
  //   auto   → 有 apiKey 用 direct（前端直连，仅开发者工具可用），无则 mock
  //   mock   → 全部走本地预置回答（断网演示兜底）
  //   direct → 前端直连 openai-next（需勾选「不校验合法域名」，已在 project.config 配好）
  //   cloud  → 微信云开发云函数（需正式 AppID 部署后启用）
  mode: 'cloud',

  models: {
    // ⚠️ 仅 direct 模式用；cloud 模式真正生效的是各云函数内写死的模型
    chat: 'claude-sonnet-5',          // 情感陪伴主对话 + 看图
    vision: 'claude-sonnet-5',        // 伤情图片多模态
    reason: 'deepseek-r1',            // 风险评估推理
    rag: 'deepseek-v3',               // 法律 RAG 生成
    statement: 'claude-sonnet-5',     // 书面陈述长文
